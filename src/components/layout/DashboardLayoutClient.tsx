...
export function DashboardLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  const setActiveCompany = useAppStore(state => state.switchCompany);
  ...
  return (
    <Layout>
      {(() => {
        if (loading || isLoggingOut) return children;
        ...
        if (user?.is_suspended) {
          return (
            <div className="flex h-[80vh] items-center justify-center bg-gray-50 p-4 text-center">
